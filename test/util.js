    ...
    static algebraElement(key, args) {
        switch (key) {
            ...
            case Algebra.LEFT_JOIN: return args[2] === true ?
                { type: key, left: args[0], right: args[1] } :
                { type: key, left: args[0], right: args[1], expression: args[2] };
            ...
            case Algebra.SLICE: return args[1] === -1 ?
                { type: key, input: args[2], start: args[0] } :
                { type: key, input: args[2], start: args[0], length: args[1] };
        ...
        if (key === Algebra.AGGREGATE) {
            let result = { type: 'aggregate', symbol: args[0], expression: args[1] };
        ...
        return { type: Algebra.EXPRESSION, symbol: key, args };
    }
